...
console.log('--- Current Files in product-images ---');
...
// Renaming Logic
Object.entries(productMapping).forEach(([gen, target]) => {
    const genPath = path.join(imgDir, gen);
    const targetPath = path.join(imgDir, target);
    if (fs.existsSync(genPath)) {
        fs.renameSync(genPath, targetPath);
        console.log(`Matched Product: ${target}`);
    }
});

Object.entries(categoryMapping).forEach(([gen, target]) => {
    const genPath = path.join(imgDir, gen);
    const targetPath = path.join(baseDir, target);
    if (fs.existsSync(genPath)) {
        fs.renameSync(genPath, targetPath);
        console.log(`Matched Category: ${target}`);
    }
});
